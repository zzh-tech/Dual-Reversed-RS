...
#         self.conv0 = ConvDS(3, int(c // 2))
#         self.conv1 = ConvDS(int(c // 2), int(c // 2))
#         self.conv2 = ConvDS(int(c // 2), c)
#         self.conv3 = ConvDS(c, 2 * c)
#         self.conv4 = ConvDS(2 * c, 4 * c)
#
#     def forward(self, x):
#         x = self.conv0(x)
#         c1 = self.conv1(x)
...
class DIFENet(nn.Module):
    def __init__(self, c=16, num_flows=3):
        self.num_flows = num_flows
        super(DIFENet, self).__init__()
    ...
    def forward(self, img_t2b, img_b2t, flow_t2b, flow_b2t, c_t2b, c_b2t):
        ...
        mask = mask.reshape(n, self.num_flows, 1, h, w)  # TODO: (n, num_flows, 1, h, w) or (n, 1, num_flows, 1, h, w)
        # print(mask)
...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.ife_net = DIFENet(c=args.n_feats, num_flows=args.frames)
